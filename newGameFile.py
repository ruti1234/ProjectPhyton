...
while True:
   ...
   # pieceImage=f.makeBackgroundTransfer(pieceImage)
    ...
    if 1 in matrix[pieceLocation[0]:pieceLocation[0] + pieceHeight,
...
if game_over:
   ...
   # pieceLocation[1]:pieceLocation[1] + pieceWidth] = pieceImage
    ...
    # Clear the region where the points text is displayed
    cv2.rectangle(canvasImage_combined, (20, 10), (200, 60), (0, 0, 0), cv2.FILLED)

    # Add the new points text
    cv2.putText(canvasImage_combined, f"Points: {points}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2,
                cv2.LINE_AA)
    ...
    color = (0, 0, 255)
    thickness = 5
    canvasImage = cv2.putText(canvasImage_combined, 'Game Over', org, cv2.FONT_HERSHEY_SIMPLEX, fontScale, color,
